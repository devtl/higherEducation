...
df <- data.world::query(data.world::qry_sql("SELECT * FROM clean_merged_university_data LIMIT 1000"), dataset = project)
...
mapdf <- aggregate(in_state_tuition ~ state, data = df, FUN = mean)
repWith <- c("alabama", "alaska", "arizona", "arkansas", "california", "colorado", "connecticut", 
...
rep <- tibble(state = rep, id = repWith)

mapdf <- merge(mapdf,rep, by = "state")
mapdf <- merge(mapdf,fifty_states, by = "id")

data("fifty_states") # this line is optional due to lazy data loading

# map_id creates the aesthetic mapping to the state name column in your data
renderPlot({
  ggplot(mapdf, aes(map_id = id)) + 
